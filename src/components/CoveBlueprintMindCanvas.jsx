...
export default function CoveBlueprintMindCanvas() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < tendrilCount; i++) {
      tendrils.push({
        ...
        speed: 0.00015 + Math.random() * 0.0002,      // Ultra slow-mo pace
        phaseOffset: Math.random() * Math.PI * 2,
        waveFreq: 0.5 + Math.random() * 0.5,           // Very low frequency
    ...
    for (let i = 0; i < nodesCount; i++) {
      nodes.push({
        ...
        vx: (Math.random() - 0.5) * 0.1, // Ultra slow particle drift
    ...
    let time = 0;
    const render = () => {
      time += 0.004; // ULTRA SLOW-MO TIME STEP (Zero distraction!)
      ...
      // Adaptive Color & Stroke Scheme (THICKER VECTORS FOR FROSTED GLASS)
      const colors = isLightMode ? {
        grid: 'rgba(15, 23, 42, 0.09)',
        tendril: 'rgba(5, 150, 105, 0.55)',
        orb: 'rgba(5, 150, 105, 0.9)',
        badge: 'rgba(5, 150, 105, 0.9)',
        strokeVerified: 'rgba(5, 150, 105, 0.65)',
        strokeUnverified: 'rgba(15, 23, 42, 0.35)',
      ...
      // 2. Render Fluid Synaptic Tendrils with Ultra Slow-Mo Waves
      tendrils.forEach((t, idx) => {
        ...
        // Slow-mo Bioluminescent Energy Orb
        const tParam = (Math.sin(time * 0.4 + idx) + 1) / 2;
      ...
      // 3. Render Architectural Blueprints with Thicker Glass Vectors & Ultra Slow-Mo Drift
      schematics.forEach((sch, idx) => {
        const isVerified = verifiedNodes.has(sch.id);

        // Ultra slow-mo drift amplitude & frequency (Zero distraction!)
        ...
        // Blueprint Vector Stroke (SLIGHTLY THICKER FOR GLASS LEGIBILITY)
        ctx.strokeStyle = isVerified ? colors.strokeVerified : colors.strokeUnverified;
        ctx.lineWidth = isVerified ? (isLightMode ? 2.2 : 1.4) : (isLightMode ? 1.8 : 1.0);
        ctx.setLineDash([4, 4]);
